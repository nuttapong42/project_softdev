<?php 
session_start();
include('server.php');
include ('menu.php');

 ?>

 <!DOCTYPE html>
 <html>
 <head>
 	<title></title>
 	<meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0" />
    <link rel="shortcut icon" href="./img/favicon.png" />
    <meta name="og:type" content="website" />
    <meta name="twitter:card" content="photo" />
    <link rel="stylesheet" type="text/css" href="css/maps-pages-1-of-2.css" />
 </head>
 	<div class="row">
	<div class="col-sm-4"></div>
	<div class="col-sm-4">
 <body background="img/bg.png">
 <input type="hidden" id="anPageName" name="page" value="maps-pages-1-of-2" />
    <div class="anima-container-center-horizontal">
      <div class="maps-pages-1-of-2 anima-screen">
        
      
        <img class="rectangle-6-C61RwL" src="img/rectangle-6@1x.png" />
        
        <div class="station-dot-C61RwL">
          <img class="line-1-BlLbCg" src="img/line-1@1x.png" />
          <img class="line-2-BlLbCg" src="img/line-1@1x.png" />
          <img class="line-3-BlLbCg" src="img/line-1@1x.png" />
          <img class="path-3-BlLbCg" src="img/line-1@1x.png" />
          <img class="line-5-BlLbCg" src="img/line-1@1x.png" />
          <img class="path-4-BlLbCg" src="img/line-1@1x.png" />
          <img class="path-2-BlLbCg" src="img/line-1@1x.png" />
          <div class="group-95-BlLbCg anima-hidden">
            <img class="line-8-mA8TeR" src="img/line-10@1x.png" />
            <img class="line-9-mA8TeR" src="img/line-10@1x.png" />
            <img class="line-10-mA8TeR" src="img/line-10@1x.png" />
            <img class="line-11-mA8TeR" src="img/line-10@1x.png" />
            <img class="line-12-mA8TeR" src="img/line-10@1x.png" />
            <img class="line-13-mA8TeR" src="img/line-10@1x.png" />
            <img class="line-14-mA8TeR" src="img/line-10@1x.png" />
            <img class="line-15-mA8TeR" src="img/line-10@1x.png" />
          </div>
        </div>
        <a href="phaya-thai.html">
          <div class="group-104-C61RwL">
            <img class="path-1-yXv3Oh" src="img/path-1@1x.png" />
            <div class="phaya-thai-yXv3Oh font-class-3">Phaya Thai</div>
          </div></a
        >
        <a href="makkasan.html">
          <div class="group-102-C61RwL">
            <img class="ellipse-4-eYqyUC" src="img/ellipse-3@1x.png" />
            <div class="makkasan-eYqyUC font-class-3">Makkasan</div>
          </div></a
        >
        <a href="hua-mak.html">
          <div class="group-100-C61RwL">
            <img class="ellipse-6-lvfzGr" src="img/ellipse-3@1x.png" />
            <div class="hua-mak-lvfzGr font-class-3">Hua Mak</div>
          </div></a
        >
        <a href="latkrabang.html">
          <div class="group-96-C61RwL">
            <img class="ellipse-8-3U5qE3" src="img/ellipse-3@1x.png" />
            <div class="latkrabang-3U5qE3 font-class-3">Latkrabang</div>
          </div></a
        >
        <a href="ratchaprarop.html">
          <div class="group-103-C61RwL">
            <img class="ellipse-3-B08mCn" src="img/ellipse-3@1x.png" />
            <div class="ratchaprarop-B08mCn font-class-3">Ratchaprarop</div>
          </div></a
        >
        <a href="ramkhamhaeng.html">
          <div class="group-101-C61RwL">
            <img class="ellipse-5-wAIKMX" src="img/ellipse-3@1x.png" />
            <div class="ramkhamhaeng-wAIKMX font-class-3">Ramkhamhaeng</div>
          </div></a
        >
        <a href="ban-thap-chang.html">
          <div class="group-99-C61RwL">
            <img class="ellipse-7-ZnSQrh" src="img/ellipse-3@1x.png" />
            <div class="ban-thap-chang-ZnSQrh font-class-3">Ban Thap Chang</div>
          </div></a
        >
        <a href="suvarnabhumi.html">
          <div class="group-98-C61RwL">
            <img class="ellipse-9-royatl" src="img/ellipse-3@1x.png" />
            <div class="suvarnabhumi-royatl font-class-3">Suvarnabhumi</div>
          </div></a
        >
        
        <div class="group-97-C61RwL">
          <div class="a8-ixNXIO font-class-3">A8</div>
          <div class="a7-ixNXIO font-class-3">A7</div>
          <div class="a6-ixNXIO font-class-3">A6</div>
          <div class="a5-ixNXIO font-class-3">A5</div>
          <div class="a4-ixNXIO font-class-3">A4</div>
          <div class="a3-ixNXIO font-class-3">A3</div>
          <div class="a2-ixNXIO font-class-3">A2</div>
          <div class="a1-ixNXIO font-class-3">A1</div>
        </div>
      </div>
    </div>
  </body>
</html>